...
mochaSuiteFn('tracing/cloud/aws-sdk/v3/sqs', function () {
  ...
  describe('tracing enabled, no suppression', function () {
    ...
    receivingMethods.forEach(sqsReceiveMethod => {
      describe.only(`receiving via ${sqsReceiveMethod} API`, () => {
    ...
    describe.only('messages sent in batch', () => {
      receivingMethods.forEach(sqsReceiveMethod => {
  ...
  describe.only('tracing disabled', () => {
  ...
  describe.only('tracing enabled but suppressed', () => {
    const senderControls = new ProcessControls({
      appPath: path.join(__dirname, 'sender'),
      port: 3215,
      useGlobalAgent: true,
